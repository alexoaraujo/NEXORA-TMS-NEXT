...
BEGIN
  ...
  IF (SELECT rolcreaterole FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'runtime role has CREATEROLE';
  END IF;
  IF (SELECT rolcreatedb FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'runtime role has CREATEDB';
  END IF;
END
$$;
...
DO $$
BEGIN
  IF (SELECT count(*) FROM identity.tenant WHERE id IN (
...
DO $$
BEGIN
  IF (SELECT count(*) FROM transport.order WHERE id=(SELECT id FROM p0_2_ids)) <> 1 THEN
    RAISE EXCEPTION 'Tenant A cannot read own order';
  END IF;
END
$$;
...
DO $$
BEGIN
  IF (SELECT count(*) FROM transport.order WHERE id=(SELECT id FROM p0_2_ids)) <> 0 THEN
...
BEGIN
  IF (SELECT count(*) FROM transport.order WHERE id=(SELECT id FROM p0_2_ids)) <> 0 THEN
    RAISE EXCEPTION 'Tenant B sees Tenant A order';
  END IF;
END
$$;
...
BEGIN
  IF (SELECT count(*)
      FROM audit.event
      WHERE entity_id=(SELECT id FROM p0_2_ids)
        AND tenant_id=identity.current_tenant_id()
        AND action='TRANSPORT_ORDER_CREATED') <> 1 THEN
    RAISE EXCEPTION 'missing canonical audit event';
  END IF;

  IF (SELECT count(*)
      FROM event.outbox
      WHERE aggregate_id=(SELECT id FROM p0_2_ids)
        AND tenant_id=identity.current_tenant_id()
        AND event_type='transport.order.created') <> 1 THEN
    RAISE EXCEPTION 'missing canonical outbox event';
  END IF;
END
$$;
...
BEGIN
  IF (
    ...
    WHERE tenant_id=identity.current_tenant_id()
      AND key='p0-2-idempotency'
      AND endpoint='test://p0-2'
  ) <> 1 THEN
